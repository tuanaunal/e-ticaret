    ...
    $.ajax({
      ...
      success: function(resp){
        ...
        if (data.status === 'success'){
          ...
          show('Adet artırıldı', 'success');
          setTimeout(function(){ location.reload(); }, 500);
        } else {
          show(data.message || data.mesaj || 'Adet artırılamadı.', 'danger', 4000);
        }
      },
      error: function(xhr){
        show('Ağ hatası: ' + (xhr.responseText || ''), 'danger', 4000);
      }
    });
  });
  ...
  $(document).on('click', '.sepet-azalt', function(e){
    ...
    $.ajax({
      ...
      success: function(resp){
        ...
        if (data.status === 'success'){
          if (typeof data.toplam_adet !== 'undefined') $('#sepet-sayi').text(data.toplam_adet);
          show('Adet azaltıldı', 'secondary');
          setTimeout(function(){ location.reload(); }, 500);
        } else {
          show(data.message || data.mesaj || 'Adet azaltılamadı.', 'danger', 4000);
        }
      },
      error: function(xhr){
        show('Ağ hatası: ' + (xhr.responseText || ''), 'danger', 4000);
