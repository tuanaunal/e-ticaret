...
$q->execute();
$sepetRows = $q->get_result()->fetch_all(MYSQLI_ASSOC);
$q->close();
if (empty($sepetRows)) {
    header("Location: siparis_form.php?hata=sepet");
    exit();
...
try {
    ...
    $upd = $conn->prepare("UPDATE urun SET stok = stok - ? WHERE urun_id = ? AND stok >= ?");
    foreach ($sepetRows as $row) {
        $uid  = (int)$row['urun_id'];
        $adet = max(1, (int)$row['adet']);

        $upd->bind_param("iii", $adet, $uid, $adet);
        $upd->execute();

        if ($upd->affected_rows !== 1) {
            $conn->rollback();
            header("Location: siparis_form.php?hata=stok");
            exit();
        }
    }
    $upd->close();
